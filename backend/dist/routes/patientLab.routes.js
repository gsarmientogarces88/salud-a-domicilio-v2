...
router.post('/', upload_1.orderUpload.single('orderFile'), async (req, res) => {
    try {
        ...
        const { patientName, examRequested, address, region, province, commune, phone, email, observationsPatient, preferredDate, preferredTimeRange, latitude, longitude, } = req.body;
        if (!patientName || !examRequested || !address || !region || !province || !commune || !phone || !email) {
            return res.status(400).json({
                error: true,
                message: 'Campos requeridos: patientName, examRequested, address, region, province, commune, phone, email',
            });
        }
        ...
        const quoteDeadlineMinutes = Math.max(1, config_1.config.labExams.quoteDeadlineMinutes);
        ...
        const compatibleLabs = await prisma_1.default.laboratory.findMany({
            where: {
        ...
        await (0, labExamHelpers_1.addLabEvent)(created.id, 'QUOTE_WINDOW_OPEN', `Cotizaciones abiertas por ${quoteDeadlineMinutes} minutos (${compatibleLabs.length} laboratorio(s) compatible(s)).`);
...
router.post('/:id/accept-quote', async (req, res) => {
    try {
        ...
        if (!quoteId)
            return res.status(400).json({ error: true, message: 'quoteId es requerido' });
        const r = await prisma_1.default.labExamRequest.findFirst({
            where: { id: req.params.id, patientId },
            include: { quotes: true },
        });
        ...
        if (uid) {
            await (0, labNotifications_service_1.notifyUser)(uid, 'LAB_QUOTE', 'Cotización aceptada', `Aceptaste una cotización de ${(0, labExamHelpers_1.formatLabDisplayId)(r.displayNumber)}. El laboratorio coordinará la visita.`, `/dashboard/patient/examenes-domicilio`);
        }
        const full = await prisma_1.default.labExamRequest.findUnique({
            where: { id: r.id },
            include: {
...
router.post('/:id/reject-quote', async (req, res) => {
    try {
        ...
        await prisma_1.default.labQuote.updateMany({
            where: { requestId: r.id, status: 'SENT' },
            data: { status: 'REJECTED' },
        });
        await (0, labExamHelpers_1.addLabEvent)(r.id, 'QUOTE_REJECTED', 'Cotizaciones rechazadas por el paciente');
        const full = await prisma_1.default.labExamRequest.findUnique({
            where: { id: r.id },
            include: {
                selectedQuote: { include: { laboratory: { select: { id: true, name: true, phone: true } } } },
